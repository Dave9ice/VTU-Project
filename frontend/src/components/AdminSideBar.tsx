import { FaTachometerAlt, FaWifi } from "react-icons/fa";
...
const AdminSideBar = () => {
  ...
  return (
    <aside
      className={`w-[15rem] fixed h-screen bg-secondary  top-0 left-0 z-50 transform transition-transform duration-300 -translate-x-full  ${
    ...
    >
      ...
      <main className=" grid bg-secondary">
        ...
        <div className="bg-transparent transform transition-transform duration-150   justify-start py-4 text-gray-600 border-b-2 capitalize hover:underline hover:translate-x-1.5 hover:text-gray-400 p-4">
          <Link to={`/admin/transactions`} className="flex gap-4 items-center ">
            {<FaWifi />}
            <h2>transactions</h2>
          </Link>
        </div>
